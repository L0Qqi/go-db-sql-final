...
func (s ParcelStore) Get(number int) (Parcel, error) {
	...
	row, err := s.db.Query("SELECT number, client, status, adress, created_at FROM parcel WHERE number = :number", sql.Named("number", number))
	if err != nil {
		return Parcel{}, err
	}
	defer row.Close()
...
func (s ParcelStore) GetByClient(client int) ([]Parcel, error) {
	...
	rows, err := s.db.Query("SELECT number, client, status, adress, created_at FROM parcel WHERE client = :client", sql.Named("client", client))
	if err != nil {
		return []Parcel{}, err
	}
	defer rows.Close()
...
func (s ParcelStore) SetAddress(number int, address string) error {
	...
	_, err = s.db.Exec("UPDATE parcel SET adress = :address WHERE number = :number",
